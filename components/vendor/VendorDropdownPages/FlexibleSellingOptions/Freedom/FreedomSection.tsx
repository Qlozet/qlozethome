...
export function FreedomSection({ data }: FreedomSectionProps) {
  const modules = [
    ...
    { icon: Users, label: "Customers", value: "89", color: "bg-[#F9F9F8] border-[#3A3A3A]/10", dot: "bg-[#F9F9F8]0" },
        ...
        <div className="flex flex-col gap-16 lg:flex-row-reverse lg:items-center lg:gap-24">
            ...
            <div className="grid gap-4">
              {data.features.map((feature, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 + i * 0.1 }} className="flex items-center gap-6 group">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#F9F9F8] border border-[#3A3A3A]/5 group-hover:bg-[#3A3A3A] group-hover:text-white transition-all shadow-sm"><Zap className="h-4 w-4" strokeWidth={1.5} /></div>
          ...
          <div className="relative lg:w-1/2 order-2 lg:order-1">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative mx-auto w-full max-w-md rounded-[2.5rem] bg-white border border-[#3A3A3A]/[0.06] shadow-2xl overflow-hidden">
              ...
              {/* Header */}
              <div className="px-8 pt-8 pb-5 flex items-center justify-between border-b border-[#3A3A3A]/5">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-xl bg-[#3A3A3A] text-white flex items-center justify-center shadow-lg">
                    <Zap className="h-4 w-4" />
                  </div>
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-[#3A3A3A]">Dashboard</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="relative cursor-pointer">
                    <Bell className="h-4 w-4 text-[#3A3A3A]/25 hover:text-[#3A3A3A]/50 transition-colors" />
              ...
              <motion.div
                ...
                className="mx-6 mt-5 h-11 rounded-xl bg-[#F9F9F8] border border-[#3A3A3A]/[0.04] px-4 flex items-center gap-3 cursor-pointer hover:border-[#3A3A3A]/15 transition-colors"
              >
                <Search className="h-3.5 w-3.5 text-[#3A3A3A]/15" />
                <span className="font-ui text-[9px] text-[#3A3A3A]/15">Search anything...</span>
              </motion.div>
                ...
                {modules.map((mod, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="h-9 w-9 rounded-xl bg-white/80 shadow-sm flex items-center justify-center group-hover:bg-[#3A3A3A] group-hover:text-white transition-all duration-300">
                        <mod.icon className="h-4 w-4" strokeWidth={1.5} />
                      </div>
                      <ChevronRight className="h-3 w-3 text-[#3A3A3A]/10 group-hover:text-[#3A3A3A]/30 group-hover:translate-x-0.5 transition-all" />
                    </div>
                    <div>
                      <span className="font-display text-2xl font-bold text-[#3A3A3A] tracking-tight">{mod.value}</span>
                      <span className="font-display text-[8px] font-bold uppercase tracking-widest text-[#3A3A3A]/20 ml-2">{mod.label}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="px-6 pb-5">
                <span className="font-display text-[8px] font-bold uppercase tracking-widest text-[#3A3A3A]/15 mb-3 block">Quick Actions</span>
                <div className="flex gap-2">
                  {["New Product", "View Orders", "Analytics"].map((action, i) => (
                    <motion.div
                      ...
                      className="flex-1 h-10 rounded-xl bg-[#F9F9F8] border border-[#3A3A3A]/[0.04] flex items-center justify-center gap-2 cursor-pointer hover:bg-[#3A3A3A] hover:text-white hover:border-transparent transition-all duration-300 group"
                    >
                      <CheckCircle2 className="h-3 w-3 text-[#3A3A3A]/15 group-hover:text-white/60 transition-colors" />
                      <span className="font-display text-[7px] font-bold uppercase tracking-wider text-[#3A3A3A]/30 group-hover:text-white transition-colors">{action}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
              ...
              {/* Footer */}
              <div className="px-8 py-4 bg-[#F9F9F8] border-t border-[#3A3A3A]/5 flex items-center justify-center">
                <span className="font-display text-[7px] font-bold uppercase tracking-[0.4em] text-[#3A3A3A]/15">Everything • One Place • Simple</span>
              </div>
            </motion.div>
          </div>
        </div>
