...
const MODES = [
  {
    ...
    tag: "Fastest",
    tagStyle: "bg-brand-light text-brand-darker border border-brand-darker/20",
    accentColor: "bg-brand-darker",
    ...
    steps: [
      {
        ...
        statusColor: "bg-brand-light text-brand-darker/40",
        active: true,
      },
      {
        icon: Zap,
      ...
      },
      {
        ...
        label: "Production",
        note: "You begin work",
        statusColor: "bg-brand-light text-[#111111]/30",
        active: false,
      },
    ],
    orderPreview: {
      ...
      <div className="mx-auto max-w-[94rem] px-6 md:px-10 lg:px-10">
        <div className="flex flex-col gap-16 lg:flex-row lg:items-start lg:gap-28">
          ...
          <div className="flex flex-col gap-10 lg:w-[44%]">
            <div className="flex flex-col gap-6">
              ...
              <motion.p
                ...
                className="max-w-xl font-ui text-base leading-relaxed text-[#111111]/40 sm:text-lg"
              >
                {data.description}
              </motion.p>
            </div>
            ...
            <div className="grid gap-4">
              {data.features.map((feature, i) => (
                ...
                >
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-light border border-brand-darker/5 group-hover:bg-brand-darker group-hover:text-white transition-all shadow-sm flex-shrink-0">
            ...
            >
              <span className="font-display text-[8px] font-bold uppercase tracking-[0.4em] text-brand-darker/20">Choose a workflow mode to see how it works</span>
                ...
                {MODES.map((m, i) => (
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 h-11 rounded-xl font-display text-[9px] font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer border ${
                      active === i
                        ? 'bg-brand-darker text-white border-brand-darker shadow-lg'
                        : 'bg-brand-light text-brand-darker/30 border-brand-darker/[0.06] hover:border-brand-darker/20 hover:text-brand-darker/60'
                    }`}
                  >
          ...
          <div className="relative lg:w-[56%]">
            ...
            >
              {/* Card */}
              <div className="rounded-[2.5rem] border border-brand-darker/[0.07] bg-white shadow-2xl shadow-black/[0.07] overflow-hidden">

                {/* Card header — mode title + tag */}
                <div className="px-7 pt-7 pb-5 border-b border-brand-darker/[0.05]">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <div className="flex items-center gap-3.5">
                        <div className="h-10 w-10 rounded-xl bg-brand-darker text-white flex items-center justify-center shadow-sm">
                          <mode.icon className="h-4.5 w-4.5" strokeWidth={1.5} />
                        </div>
                        <div className="flex flex-col gap-0.5">
                          <span className="font-display text-sm font-bold tracking-tight text-[#111111]">{mode.fullLabel}</span>
                          <span className="font-ui text-[9px] text-[#111111]/30">{mode.summary}</span>
                        </div>
                      </div>
                ...
                {/* Live order preview */}
                <div className="px-7 py-5 border-b border-brand-darker/[0.05] bg-brand-light">
                  <span className="font-display text-[7px] font-bold uppercase tracking-[0.4em] text-[#111111]/20 mb-3 block">Live Order Example</span>
                  <AnimatePresence mode="wait">
                    <motion.div
                      key={active}
                      ...
                      className="flex items-center justify-between rounded-2xl bg-white border border-brand-darker/[0.06] shadow-sm px-5 py-4 group hover:shadow-md transition-shadow"
                    >
                      <div className="flex items-center gap-4">
                        <div className="h-10 w-10 rounded-xl bg-brand-light border border-brand-darker/5 flex items-center justify-center flex-shrink-0">
                          <Package className="h-4 w-4 text-brand-darker/20" strokeWidth={1.5} />
                        </div>
                        <div className="flex flex-col gap-0.5">
                          <span className="font-display text-[11px] font-bold text-[#111111]/70">{mode.orderPreview.item}</span>
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-[8px] text-[#111111]/20">{mode.orderPreview.id}</span>
                            <span className="font-mono text-[8px] text-[#111111]/15">·</span>
                            <span className="font-mono text-[8px] text-[#111111]/20">{mode.orderPreview.size}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        ...
                        <div className="flex items-center gap-1">
                          <Clock className="h-2.5 w-2.5 text-[#111111]/10" />
                          <span className="font-mono text-[7px] text-[#111111]/15">{mode.orderPreview.time}</span>
                        </div>
                      </div>
                    </motion.div>
                  </AnimatePresence>
                ...
                <div className="px-7 py-5">
                  <span className="font-display text-[7px] font-bold uppercase tracking-[0.4em] text-[#111111]/20 mb-4 block">How it flows</span>
                  <AnimatePresence mode="wait">
                    <motion.div
                      key={active}
                      ...
                      {mode.steps.map((step, si) => (
                        <div key={si} className="flex items-center flex-1 min-w-0">
                          ...
                          <div className={`flex-1 flex flex-col items-center text-center gap-2 px-2 py-4 rounded-2xl transition-all duration-300 ${
                            step.active
                              ? 'bg-brand-light border border-brand-darker/[0.05]'
                              : 'bg-white border border-dashed border-brand-darker/[0.05] opacity-40'
                          }`}>
                            <div className={`h-9 w-9 rounded-xl flex items-center justify-center ${
                              step.active
                                ? 'bg-brand-darker text-white shadow-md'
                                : 'bg-brand-light text-brand-darker/20'
                            }`}>
                              <step.icon className="h-4 w-4" strokeWidth={1.5} />
                            </div>
                            <div className="flex flex-col gap-0.5">
                              <span className="font-display text-[8px] font-bold uppercase tracking-wider text-[#111111]/60 leading-tight">{step.label}</span>
                              <span className="font-ui text-[7px] text-[#111111]/20 leading-snug">{step.note}</span>
                            </div>
                            {step.active && (
                          ...
                          {si < mode.steps.length - 1 && (
                            <div className="flex-shrink-0 px-1">
                              <ArrowRight className="h-3 w-3 text-brand-darker/10" />
                            </div>
                          )}
                        </div>
                      ))}
                ...
                <div className="px-7 pb-7">
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="rounded-2xl bg-brand-darker text-white px-5 py-4 flex items-center gap-4"
                    >
              ...
              >
                <span className="font-display text-[7px] font-bold uppercase tracking-[0.4em] text-[#111111]/15">
