      ...
      content: (
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { name: "Wool Jacket", price: "$210", color: "bg-[#F9F9F8] border-[#3A3A3A]/10" },
          ].map((item, i) => (
            <motion.div
              key={i}
            ...
            >
              ...
              <span className="font-display text-[9px] font-bold text-[#3A3A3A]/60 uppercase tracking-wider">{item.name}</span>
              <span className="font-display text-sm font-bold text-[#3A3A3A]">{item.price}</span>
            </motion.div>
          ))}
        </div>
      ),
      ...
      content: (
        <div className="flex flex-col gap-3">
          {[
            ...
            >
              <span className="font-display text-[9px] font-bold uppercase tracking-wider text-[#3A3A3A]/30 w-14 shrink-0">{m.label}</span>
              <div className="flex-1 h-3 bg-zinc-100 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${m.pct}%` }}
                  transition={{ delay: 0.2 + i * 0.1, duration: 0.6 }}
                  className="h-full bg-[#3A3A3A] rounded-full group-hover:bg-[#F9F9F8]0 transition-colors"
                />
              </div>
              <span className="font-mono text-[10px] font-bold text-[#3A3A3A] w-8 text-right">{m.val}</span>
            </motion.div>
          ))}
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="flex items-center gap-2 mt-2 bg-[#F9F9F8] border border-[#3A3A3A]/20 rounded-xl px-4 py-3"
          >
            <CheckCircle2 className="h-3.5 w-3.5 text-[#3A3A3A]" />
            <span className="font-display text-[8px] font-bold text-[#3A3A3A] uppercase tracking-wider">Custom Profile Saved</span>
          </motion.div>
        </div>
      ),
    },
      ...
      content: (
        <div className="flex flex-col gap-3">
          <div className="flex gap-3">
            <div className="flex-1 rounded-xl bg-[#F9F9F8] border border-[#3A3A3A]/[0.04] p-4 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <ShoppingBag className="h-3 w-3 text-[#3A3A3A]/30" />
                <span className="font-display text-[8px] font-bold uppercase tracking-wider text-[#3A3A3A]/40">Ready-Made</span>
              </div>
              <span className="font-display text-2xl font-bold text-[#3A3A3A]">14</span>
              <span className="font-display text-[7px] text-[#3A3A3A]/20 uppercase tracking-wider">Products Live</span>
            </div>
            <div className="flex-1 rounded-xl bg-[#F9F9F8] border border-[#3A3A3A]/[0.04] p-4 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <Ruler className="h-3 w-3 text-[#3A3A3A]/30" />
                <span className="font-display text-[8px] font-bold uppercase tracking-wider text-[#3A3A3A]/40">Custom</span>
              </div>
              <span className="font-display text-2xl font-bold text-[#3A3A3A]">8</span>
              <span className="font-display text-[7px] text-[#3A3A3A]/20 uppercase tracking-wider">Active Orders</span>
            </div>
          </div>
          <motion.div
            ...
            className="h-14 rounded-xl bg-[#3A3A3A] text-white px-5 flex items-center justify-between group cursor-pointer hover:bg-zinc-800 transition-colors"
          >
          ...
          <div className="flex gap-2">
            {["Unified Dashboard", "One Catalog", "Seamless UX"].map((t, i) => (
              <motion.span
                ...
                className="flex-1 text-center py-2 rounded-lg bg-[#F9F9F8] border border-[#3A3A3A]/[0.04] font-display text-[7px] font-bold uppercase tracking-wider text-[#3A3A3A]/25"
              >
                {t}
              </motion.span>
            ))}
            ...
            <div className="grid gap-4">
              {data.features.map((feature, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 + i * 0.1 }} className="flex items-center gap-6 group">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#F9F9F8] border border-[#3A3A3A]/5 group-hover:bg-[#3A3A3A] group-hover:text-white transition-all shadow-sm"><CheckCircle2 className="h-4 w-4" strokeWidth={1.5} /></div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative mx-auto w-full max-w-md rounded-[2.5rem] bg-white border border-[#3A3A3A]/[0.06] shadow-2xl overflow-hidden">
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 h-10 rounded-lg font-display text-[8px] font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                        activeMode === i
                          ...
                          : 'text-[#3A3A3A]/30 hover:text-[#3A3A3A]/60'
                      }`}
                    >
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-5">
                      <div className="flex flex-col gap-0.5">
                        <span className="font-display text-[11px] font-bold uppercase tracking-wider text-[#3A3A3A]">{modes[activeMode].label}</span>
                        <span className="font-ui text-[8px] text-[#3A3A3A]/25">{modes[activeMode].desc}</span>
                      </div>
              ...
              <div className="px-6 py-4 bg-[#F9F9F8] border-t border-[#3A3A3A]/5 flex items-center justify-between">
                <div className="flex gap-1.5">
                  {modes.map((_, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${activeMode === i ? 'w-6 bg-[#3A3A3A]' : 'w-1.5 bg-[#3A3A3A]/10'}`} />
                  ))}
                </div>
                <span className="font-display text-[7px] font-bold uppercase tracking-[0.4em] text-[#3A3A3A]/15">Switch anytime</span>
              </div>
            </motion.div>
          </div>
        </div>
