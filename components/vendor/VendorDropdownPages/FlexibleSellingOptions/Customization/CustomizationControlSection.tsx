        ...
        <div className="flex flex-col gap-16 lg:flex-row-reverse lg:items-center lg:gap-32">
            ...
            <div className="grid gap-4">
              {data.features.map((feature, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 + i * 0.1 }} className="flex items-center gap-6 group">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-zinc-50 border border-black/5 group-hover:bg-black group-hover:text-white transition-all shadow-sm"><Scissors className="h-4 w-4" strokeWidth={1.5} /></div>
          ...
          <div className="relative lg:w-1/2 order-2 lg:order-1">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative mx-auto w-full max-w-md rounded-[2.5rem] bg-white border border-black/[0.06] shadow-2xl overflow-hidden">
              ...
              {/* Top Bar */}
              <div className="px-8 pt-8 pb-5 border-b border-black/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-black">Style Controls</span>
                </div>
                <span className="font-mono text-[9px] text-black/20">{options.filter(o => o.enabled).length}/{options.length} open</span>
              </div>
                  ...
                  <motion.div
                    ...
                    className={`rounded-2xl border transition-all duration-500 overflow-hidden ${
                      opt.enabled 
                        ...
                        : 'bg-zinc-50 border-black/[0.03] opacity-50'
                    }`}
                  >
                    {/* Option Header */}
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className={`h-8 w-8 rounded-lg flex items-center justify-center transition-all cursor-pointer ${
                            opt.enabled ? 'bg-black text-white shadow-lg' : 'bg-zinc-200 text-zinc-400'
                          }`}
                        >
                          {opt.enabled ? <Unlock className="h-3.5 w-3.5" /> : <Lock className="h-3.5 w-3.5" />}
                        </button>
                        <span className="font-display text-[11px] font-bold uppercase tracking-wider text-black/70">{opt.label}</span>
                      </div>
                      {opt.enabled && (
                        <motion.span 
                          initial={{ opacity: 0 }} 
                          animate={{ opacity: 1 }}
                          className="font-display text-[9px] font-bold text-black/30 uppercase tracking-wider"
                        >
                            ...
                            <button
                              ...
                              className={`flex-1 h-9 rounded-xl font-display text-[8px] font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                                ci === opt.selected
                                  ...
                                  : 'bg-zinc-50 text-black/30 border border-black/5 hover:text-black/60 hover:border-black/15'
                              }`}
                            >
                              {choice}
                            </button>
              ...
              {/* Bottom Summary */}
              <div className="px-8 py-5 bg-zinc-50 border-t border-black/5 flex items-center justify-between">
                <div className="flex gap-1.5">
                  {options.map((o, i) => (
                    <div key={i} className={`h-2 w-2 rounded-full transition-colors ${o.enabled ? 'bg-emerald-500' : 'bg-zinc-200'}`} />
                  ))}
                </div>
                <span className="font-display text-[8px] font-bold uppercase tracking-widest text-black/20">Vendor controls what customers see</span>
              </div>
            </motion.div>
          </div>
        </div>
