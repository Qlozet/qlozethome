...
export function AdaptabilitySection({ data }: AdaptabilitySectionProps) {
  ...
  const profiles = [
    ...
    },
    {
      ...
      tasks: [
        { text: "Summer Collection Launch", status: "New", statusColor: "text-brand-darker bg-brand-light" },
            ...
            <div className="grid gap-4">
              {data.features.map((feature, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 + i * 0.1 }} className="flex items-center gap-6 group">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-light border border-brand-darker/5 group-hover:bg-brand-darker group-hover:text-white transition-all shadow-sm"><RefreshCw className="h-4 w-4" strokeWidth={1.5} /></div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative mx-auto w-full max-w-md rounded-[2.5rem] bg-white border border-brand-darker/[0.06] shadow-2xl overflow-hidden">
                    ...
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-2 h-10 rounded-lg transition-all duration-500 cursor-pointer ${
                        active === i
                          ...
                          : 'text-brand-darker/25 hover:text-brand-darker/50'
                      }`}
                    >
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col gap-0.5">
                        <span className="font-display text-[11px] font-bold uppercase tracking-wider text-[#111111]">{profiles[active].label}</span>
                        <span className="font-ui text-[8px] text-[#111111]/25">{profiles[active].desc}</span>
                      </div>
                    ...
                    <div className={`grid gap-2 ${profiles[active].stats.length <= 2 ? 'grid-cols-2' : profiles[active].stats.length === 3 ? 'grid-cols-3' : 'grid-cols-4'}`}>
                      {profiles[active].stats.map((stat, i) => (
                        <motion.div
                          ...
                          className="rounded-xl bg-brand-light border border-brand-darker/[0.04] p-3 flex flex-col items-center gap-0.5"
                        >
                          <span className="font-display text-lg font-bold text-[#111111]">{stat.val}</span>
                          <span className="font-display text-[6px] font-bold uppercase tracking-wider text-brand-darker/20">{stat.label}</span>
                        </motion.div>
                      ))}
                    </div>
                    ...
                    {profiles[active].tasks.map((task, i) => (
                      <motion.div
                        ...
                        className="h-14 rounded-xl bg-brand-light border border-brand-darker/[0.04] px-4 flex items-center justify-between group hover:bg-white hover:shadow-md hover:border-brand-darker/10 transition-all cursor-pointer"
                      >
                        <span className="font-display text-[9px] font-bold text-[#111111]/50 group-hover:text-[#111111]/80 transition-colors">{task.text}</span>
              ...
              <div className="px-6 py-4 bg-brand-light border-t border-brand-darker/5 flex items-center justify-between">
                <div className="flex gap-1.5">
                  {profiles.map((_, i) => (
                    <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${active === i ? 'w-6 bg-brand-darker' : 'w-1.5 bg-brand-darker/10'}`} />
                  ))}
                </div>
                <span className="font-display text-[7px] font-bold uppercase tracking-[0.4em] text-[#111111]/15">Same platform, every scale</span>
              </div>
            </motion.div>
          </div>
        </div>
