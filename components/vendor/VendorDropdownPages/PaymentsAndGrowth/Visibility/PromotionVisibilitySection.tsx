            ...
            <div className="relative mx-auto w-full max-w-[500px] rounded-[3rem] bg-brand-light border border-brand-darker/5 shadow-2xl p-6 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
               ...
               <div className="flex items-center justify-between pb-5 border-b border-brand-darker/5 mb-6">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="font-display text-[10px] font-bold uppercase tracking-widest text-brand-darker">Promotion Tools</span>
                  </div>
               ...
               >
                  <span className="font-mono text-[8px] font-bold text-brand-darker/60 uppercase tracking-widest block mb-3">Profile Visibility Score</span>
                  <div className="flex items-center justify-center gap-4 mb-3">
                     <motion.span
                        ...
                        className="font-display text-5xl font-bold text-brand-darker tracking-tight"
                     >87</motion.span>
                     <span className="font-mono text-[10px] font-bold text-brand-darker/30 uppercase">/100</span>
                  </div>
                  ...
                  {PROMO_FEATURES.map((feat, i) => {
                     ...
                     return (
                        <motion.div key={i} initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                           ...
                           className={`flex items-center gap-4 p-4 rounded-2xl border cursor-pointer transition-all duration-300 ${feat.active ? 'bg-white border-brand-darker/5 shadow-md hover:shadow-xl' : 'bg-brand-light/50 border-brand-darker/5 opacity-50'}`}
                        >
                           <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${feat.active ? 'bg-brand-darker text-white' : 'bg-zinc-100 text-brand-darker/30'}`}>
                              <Icon className="h-4 w-4" />
                           </div>
                           <div className="flex-1 flex flex-col gap-0.5">
                              <span className="font-display text-[10px] font-bold text-brand-darker uppercase tracking-wider">{feat.label}</span>
                              <span className="font-ui text-[9px] text-[#111111]/60">{feat.desc}</span>
                           </div>
                           <div className={`px-3 py-1 rounded-full ${feat.active ? 'bg-emerald-500/10' : 'bg-zinc-100'}`}>
                              <span className={`font-mono text-[8px] font-bold uppercase tracking-widest ${feat.active ? 'text-emerald-600' : 'text-brand-darker/30'}`}>
