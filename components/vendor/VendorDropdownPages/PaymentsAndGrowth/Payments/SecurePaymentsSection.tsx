            ...
            <div className="relative mx-auto w-full max-w-[500px] rounded-[3rem] bg-brand-light border border-brand-darker/5 shadow-2xl p-6 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
               ...
               <div className="flex items-center justify-between pb-5 border-b border-brand-darker/5 mb-6">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="font-display text-[10px] font-bold uppercase tracking-widest text-brand-darker">Payment Security</span>
                  </div>
               ...
               <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }}
                  className="bg-white rounded-2xl border border-brand-darker/5 shadow-md p-5 mb-5 text-center"
               >
                  ...
                  <span className="font-display text-sm font-bold text-brand-darker uppercase tracking-wider block mb-1">End-to-End Encrypted</span>
                  <span className="font-mono text-[9px] text-brand-darker/60 uppercase tracking-widest">256-bit SSL · PCI DSS Compliant</span>
               </motion.div>
               ...
               <div className="flex flex-col gap-3 mb-5">
                  ...
                  {PAYMENT_METHODS.map((method, i) => (
                     <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                        ...
                        className="flex items-center gap-4 p-3 rounded-xl bg-white border border-brand-darker/5 shadow-sm cursor-pointer hover:shadow-md transition-all"
                     >
                        <span className="text-xl">{method.icon}</span>
                        <span className="font-display text-[11px] font-bold text-brand-darker uppercase tracking-wider flex-1">{method.name}</span>
               ...
               {/* Trust Indicators */}
               <div className="flex items-center justify-center gap-6 pt-4 border-t border-brand-darker/5">
                  <div className="flex items-center gap-2">
                     <Lock className="h-3 w-3 text-brand-darker/30" />
                     <span className="font-mono text-[8px] font-bold text-brand-darker/50 uppercase tracking-widest">Escrow Protected</span>
                  </div>
                  <div className="h-3 w-px bg-brand-darker/10" />
                  <div className="flex items-center gap-2">
                     <CreditCard className="h-3 w-3 text-brand-darker/30" />
                     <span className="font-mono text-[8px] font-bold text-brand-darker/50 uppercase tracking-widest">Instant Processing</span>
                  </div>
               </div>
            </div>
          </div>
