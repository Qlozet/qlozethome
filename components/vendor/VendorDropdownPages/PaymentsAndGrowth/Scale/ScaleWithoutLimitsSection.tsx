            ...
            <div className="relative mx-auto w-full max-w-[500px] rounded-[3rem] bg-brand-light border border-brand-darker/5 shadow-2xl p-6 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
               ...
               <div className="flex items-center justify-between pb-5 border-b border-brand-darker/5 mb-6">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="font-display text-[10px] font-bold uppercase tracking-widest text-brand-darker">Global Reach</span>
                  </div>
               ...
               <div className="grid grid-cols-3 gap-3 mb-6">
                  {[
                     ...
                     >
                        <span className="font-display text-lg font-bold text-brand-darker block">{stat.value}</span>
                        <span className="font-mono text-[7px] font-bold text-brand-darker/40 uppercase tracking-widest">{stat.label}</span>
                     </motion.div>
                  ))}
               </div>
               ...
               <div className="flex flex-col gap-3">
                  ...
                  {REGIONS.map((region, i) => (
                     <motion.div key={i} initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                        ...
                        className="flex items-center gap-4 p-3.5 rounded-xl bg-white border border-brand-darker/5 shadow-sm cursor-pointer hover:shadow-md transition-all"
                     >
                        <span className="text-xl">{region.flag}</span>
                        <div className="flex-1 flex flex-col gap-0.5">
                           <span className="font-display text-[10px] font-bold text-brand-darker uppercase tracking-wider">{region.name}</span>
                           <div className="flex items-center gap-1.5">
                              <MapPin className="h-2.5 w-2.5 text-brand-darker/30" />
                              <span className="font-mono text-[8px] text-brand-darker/60 uppercase tracking-widest">Active</span>
                           </div>
                        </div>
                        <div className="flex items-center gap-1.5">
                           <Package className="h-3 w-3 text-brand-darker/30" />
                           <span className="font-mono text-[9px] font-bold text-brand-darker/80">{region.orders}</span>
                        </div>
                     </motion.div>
                  ))}
               </div>
