            ...
            <div className="relative mx-auto w-full max-w-[500px] rounded-[3rem] bg-[#F9F9F8] border border-[#3A3A3A]/5 shadow-2xl p-6 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
               ...
               <div className="flex items-center justify-between pb-5 border-b border-black/5 mb-6">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="font-display text-[10px] font-bold uppercase tracking-widest text-[#3A3A3A]">Growth Engine</span>
                  </div>
                  ...
                  {GROWTH_METRICS.map((metric, i) => {
                     ...
                     return (
                        <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                           ...
                           className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-black/5 shadow-md cursor-pointer hover:shadow-xl transition-all"
                        >
                           <div className="h-12 w-12 rounded-xl bg-[#F9F9F8] border border-black/5 flex items-center justify-center shrink-0">
                              <Icon className="h-5 w-5 text-black/40" />
                           </div>
                           <div className="flex-1 flex flex-col gap-1">
                              <span className="font-mono text-[8px] font-bold text-[#3A3A3A]/40 uppercase tracking-widest">{metric.label}</span>
                              <span className="font-display text-xl font-bold text-[#3A3A3A] tracking-tight">{metric.value}</span>
                           </div>
               ...
               <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.7 }}
                  className="bg-white rounded-2xl border border-black/5 shadow-md p-5"
               >
                  <div className="flex items-center justify-between mb-3">
                     <span className="font-mono text-[8px] font-bold text-[#3A3A3A]/40 uppercase tracking-widest">Customer Growth</span>
