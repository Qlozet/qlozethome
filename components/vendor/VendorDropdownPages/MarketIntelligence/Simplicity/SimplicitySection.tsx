        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:gap-24">
          ...
          <div className="lg:w-7/12 order-2 lg:order-1">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              className="rounded-[2rem] sm:rounded-[3rem] bg-brand-light border border-brand-darker/5 shadow-2xl p-5 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
              
              {/* Dashboard Header */}
              <div className="flex items-center justify-between pb-5 border-b border-brand-darker/5 mb-6">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-xl bg-brand-darker text-white flex items-center justify-center"><LayoutDashboard className="h-3.5 w-3.5" /></div>
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-brand-darker">Your Dashboard</span>
                </div>
                ...
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-mono text-[7px] font-bold text-[#111111]/30 uppercase tracking-widest">{kpi.label}</span>
                    </div>
                    <span className="font-display text-lg font-bold text-brand-darker block">{kpi.value}</span>
              ...
              <div className="bg-white rounded-2xl border border-brand-darker/5 p-5 shadow-sm mb-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-brand-darker">Today's Recommendations</span>
                </div>
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
                    ...
                    className="flex items-center gap-3 py-2.5 border-b border-brand-darker/5 last:border-0 group/item cursor-pointer hover:bg-brand-light/50 -mx-2 px-2 rounded-lg transition-colors">
                    ...
                    <span className="font-ui text-[10px] text-[#111111]/60 flex-1 leading-relaxed">{rec.text}</span>
                    <ArrowRight className="h-3 w-3 text-brand-darker/10 shrink-0 group-hover/item:text-brand-darker/30 transition-colors" />
                  </motion.div>
                ))}
              </div>
              ...
              {data.features.map((feature, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-5 group">
                    <div className="h-10 w-10 rounded-xl bg-brand-light border border-brand-darker/5 flex items-center justify-center group-hover:bg-brand-darker group-hover:text-brand-light transition-all shadow-sm">
                      <Icon className="h-4 w-4" strokeWidth={1.5} />
                    </div>
                    <span className="font-display text-lg font-medium text-[#111111]/70 group-hover:text-brand-darker transition-colors">{feature.title}</span>
                  </div>
                );
              })}
            </motion.div>
