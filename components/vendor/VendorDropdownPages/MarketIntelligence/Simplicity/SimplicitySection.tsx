        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:gap-24">
          ...
          <div className="lg:w-7/12 order-2 lg:order-1">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              className="rounded-[2rem] sm:rounded-[3rem] bg-[#F9F9F8] border border-[#3A3A3A]/5 shadow-2xl p-5 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
              
              {/* Dashboard Header */}
              <div className="flex items-center justify-between pb-5 border-b border-black/5 mb-6">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-xl bg-[#3A3A3A] text-white flex items-center justify-center"><LayoutDashboard className="h-3.5 w-3.5" /></div>
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-[#3A3A3A]">Your Dashboard</span>
                </div>
                ...
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-mono text-[7px] font-bold text-[#3A3A3A]/30 uppercase tracking-widest">{kpi.label}</span>
                    </div>
                    <span className="font-display text-lg font-bold text-[#3A3A3A] block">{kpi.value}</span>
              ...
              <div className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm mb-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-[#3A3A3A]">Today's Recommendations</span>
                </div>
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
                    ...
                    className="flex items-center gap-3 py-2.5 border-b border-black/5 last:border-0 group/item cursor-pointer hover:bg-[#F9F9F8]/50 -mx-2 px-2 rounded-lg transition-colors">
                    ...
                    <span className="font-ui text-[10px] text-[#3A3A3A]/60 flex-1 leading-relaxed">{rec.text}</span>
                    <ArrowRight className="h-3 w-3 text-[#3A3A3A]/10 shrink-0 group-hover/item:text-[#3A3A3A]/30 transition-colors" />
                  </motion.div>
                ))}
              </div>

              {/* Next Steps */}
              <div className="bg-[#3A3A3A] rounded-2xl p-5 shadow-sm">
