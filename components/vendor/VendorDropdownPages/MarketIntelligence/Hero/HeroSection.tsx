          ...
          <div className="flex flex-col gap-10 lg:w-1/2 lg:pt-8">
            <div className="flex flex-col gap-8">
              <motion.div variants={itemVariants} className="flex items-center gap-4">
                ...
                <span className="font-display text-[10px] font-bold uppercase tracking-[0.5em] text-black/40">{data.badge}</span>
              </motion.div>
              <motion.h1 variants={itemVariants} className="max-w-2xl font-display text-5xl font-medium leading-[1.02] tracking-tighter text-black sm:text-7xl lg:text-[6.5rem]">{data.title}</motion.h1>
              <motion.p variants={itemVariants} className="max-w-xl font-ui text-lg leading-relaxed text-black/40 sm:text-xl">{data.description}</motion.p>
            </div>

            <motion.div variants={itemVariants} className="flex flex-col gap-6 sm:flex-row sm:items-center">
              <Link href={data.cta.href} className="group relative inline-flex h-16 items-center justify-center gap-4 overflow-hidden rounded-full bg-black px-12 text-[10px] font-bold uppercase tracking-[0.4em] text-white transition-all hover:scale-[1.03] active:scale-95 shadow-2xl shadow-black/20">
              ...
              <Link href={data.secondaryCta.href} className="group inline-flex items-center gap-4 font-display text-[10px] font-bold uppercase tracking-[0.4em] text-black">
            ...
            <div className="relative mx-auto w-full max-w-[540px] rounded-[2rem] sm:rounded-[3rem] bg-zinc-50 border border-black/5 shadow-2xl p-5 sm:p-8 overflow-hidden transition-all duration-500 hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.25)] hover:-translate-y-2">
              ...
              <div className="flex items-center justify-between pb-5 border-b border-black/5 mb-6">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-xl bg-black text-white flex items-center justify-center"><BarChart3 className="h-3.5 w-3.5" /></div>
                  <span className="font-display text-[10px] font-bold uppercase tracking-widest text-black">Intelligence Overview</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {METRICS.map((m, i) => (
                  <motion.div key={m.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 + i * 0.1 }}
                    ...
                    <span className="font-mono text-[7px] font-bold text-black/30 uppercase tracking-widest">{m.label}</span>
                    <div className="font-display text-lg font-bold text-black mt-1">{m.value}</div>
              ...
              <div className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm mb-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="font-mono text-[8px] font-bold text-black/30 uppercase tracking-widest">Last 7 Days</span>
                </div>
                <div className="flex items-end gap-2 h-28">
                  {[45, 62, 38, 78, 55, 88, 72].map((h, i) => (
                    <motion.div key={i} initial={{ height: 0 }} animate={{ height: `${h}%` }} transition={{ delay: 1 + i * 0.08, duration: 0.6, ease: "easeOut" }}
                      ...
                      <div className="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-black text-white px-2 py-0.5 rounded text-[7px] font-mono font-bold whitespace-nowrap">
                        ₦{Math.round(h * 47)}K
                      </div>
                    </motion.div>
                  ))}
              ...
              <div className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm">
                <span className="font-mono text-[8px] font-bold text-black/40 uppercase tracking-widest mb-3 block">Top Products</span>
                {[
                  ...
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.4 + i * 0.1 }}
                    ...
                    <span className="font-mono text-[8px] font-bold text-black/20 w-4">{i + 1}</span>
                    <span className="font-display text-[10px] font-bold text-black uppercase tracking-wider flex-1 group-hover:text-black/70 transition-colors">{p.name}</span>
                    <div className="w-20 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${p.bar}%` }} transition={{ delay: 1.6 + i * 0.1, duration: 0.6 }} className="h-full bg-black rounded-full" />
                    </div>
                    <span className="font-mono text-[8px] font-bold text-black/40">{p.sales}</span>
                  </motion.div>
                ))}
              </div>
            </div>
