      ...
      <div className="mx-auto max-w-[94rem]">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <div className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <span className="font-mono text-[7px] font-bold text-black/30 uppercase tracking-widest">Monthly Revenue</span>
              ...
              <div className="flex justify-between mt-1">
                {["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"].map((m, i) => (
                  <span key={i} className="font-mono text-[6px] text-black/20">{m}</span>
                ))}
              </div>
            </div>
          </motion.div>
            ...
            <div className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm flex flex-col gap-0">
              {TOP_PRODUCTS.map((p, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 + i * 0.08 }}
                  className="flex items-center gap-3 py-3 border-b border-black/5 last:border-0 group/item cursor-pointer hover:bg-zinc-50/50 -mx-2 px-2 rounded-lg transition-colors">
                  <div className="h-7 w-7 rounded-lg bg-zinc-100 flex items-center justify-center">
                    <span className="font-mono text-[8px] font-bold text-black/40">{i + 1}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="font-display text-[9px] font-bold text-black uppercase tracking-wider block truncate">{p.name}</span>
                    <div className="h-1 bg-zinc-100 rounded-full mt-1.5 overflow-hidden">
                      <motion.div initial={{ width: 0 }} whileInView={{ width: `${p.bar}%` }} viewport={{ once: true }}
                        transition={{ delay: 0.6 + i * 0.1, duration: 0.6 }} className="h-full bg-black rounded-full" />
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="font-mono text-[9px] font-bold text-black block">{p.revenue}</span>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            ...
            <div className="bg-white rounded-2xl border border-black/5 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <span className="font-mono text-[7px] font-bold text-black/30 uppercase tracking-widest">Weekly Activity</span>
                <span className="font-mono text-[8px] font-bold text-black/30 uppercase tracking-widest">Peak: Friday</span>
              </div>
                ...
                {PURCHASE_PATTERNS.map((d, i) => (
                  <div key={i} className="flex-1 flex flex-col items-end justify-end">
                    <motion.div initial={{ height: 0 }} whileInView={{ height: d.value * 1.1 }} viewport={{ once: true }}
                      ...
                      className={`w-full rounded-lg transition-colors duration-300 cursor-pointer ${d.value > 80 ? 'bg-black hover:bg-black/80' : 'bg-black/10 hover:bg-black/20'}`} />
                    <span className="font-mono text-[7px] text-black/30 mt-1.5 text-center w-full">{d.day}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 flex items-center gap-2 pt-3 border-t border-black/5">
                <ArrowUpRight className="h-3 w-3 text-emerald-500" />
                <span className="font-mono text-[8px] text-black/40">Repeat rate: <span className="font-bold text-black">34%</span></span>
              </div>
            </div>
          </motion.div>
        </div>
