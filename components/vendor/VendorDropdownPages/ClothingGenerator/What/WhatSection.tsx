    ...
    <section id="what" className="relative z-10 bg-white px-6 py-20 lg:py-40" data-theme="light">
      <div className="mx-auto max-w-[94rem]">
        ...
        <div className="mb-24 flex flex-col items-center gap-8 text-center lg:mb-32">
          ...
          <h2 className="max-w-4xl font-display text-5xl font-medium leading-[1.05] tracking-tight text-black sm:text-7xl">
            {data.title}
          </h2>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 lg:gap-6">
          {data.tools.map((tool, index) => {
            ...
            return (
              <div
                key={tool.title}
                className={`group relative flex flex-col justify-end overflow-hidden rounded-[2rem] bg-[#FAFAFA] sm:rounded-[3rem] ${colSpan} ${rowSpan} ${minHeight}`}
              >
                {/* Background Ambient Hover Reveal */}
                <div className="absolute inset-0 z-0 bg-gradient-to-br from-black/5 to-transparent opacity-100 transition-opacity duration-700 group-hover:opacity-0" />
                <div className="absolute inset-0 z-0 bg-black opacity-0 transition-opacity duration-700 group-hover:opacity-[0.02]" />
                ...
                <div className="relative z-10 flex flex-col gap-8 p-8 transition-transform duration-700 group-hover:-translate-y-2 sm:p-12">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="max-w-md font-ui text-lg leading-relaxed text-black/50 transition-colors duration-700 group-hover:text-black/70">
                      {tool.description}
                    </p>
                  </div>
                </div>
