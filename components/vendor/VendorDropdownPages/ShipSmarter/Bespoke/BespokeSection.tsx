    ...
    <section ref={containerRef} id="bespoke" className="relative z-30 overflow-hidden bg-[#F9F9F8] px-6 py-24 sm:py-32">
      ...
      <div className="absolute left-0 top-1/2 z-0 flex w-full -translate-y-1/2 -rotate-3 select-none whitespace-nowrap opacity-[0.03] mix-blend-multiply">
        <motion.div
           ...
           className="flex gap-8 font-display text-[15rem] font-bold uppercase leading-none tracking-tighter text-black lg:text-[25rem]"
        >
      ...
      <div className="relative z-10 mx-auto max-w-[94rem]">
        ...
        <motion.div style={{ y: contentY }} className="mx-auto grid max-w-7xl gap-6 sm:grid-cols-2 lg:grid-cols-3 lg:gap-10">
          {data.features.map((feature) => (
            <div
              key={feature.title}
              className="group flex flex-col gap-10 bg-[#050505] p-10 transition-all duration-700 hover:-translate-y-3 hover:bg-black hover:shadow-2xl hover:shadow-black/40 lg:p-14"
            >
              {/* Image Container */}
