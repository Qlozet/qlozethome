...
export function ShowcaseSection({ data }: ShowcaseSectionProps) {
  ...
  return (
    <section ref={containerRef} className="relative w-full bg-brand-light py-24 sm:py-32" data-theme="light">
      <div className="mx-auto max-w-[94rem] px-6 md:px-10 lg:px-10">
        <div className="flex flex-col lg:flex-row lg:items-center lg:gap-24">
          ...
          <div className="flex flex-col gap-8 mt-8 lg:mt-0 lg:w-1/2">
            ...
            >
              {data.items.map((item) => (
                <div key={item} className="rounded-full border border-brand-darker/10 bg-white px-8 py-4 font-display text-[10px] font-bold uppercase tracking-widest text-[#111111] shadow-sm transition-all hover:scale-[1.05] hover:border-brand-darker/20">
                  {item}
                </div>
              ))}
            </motion.div>
