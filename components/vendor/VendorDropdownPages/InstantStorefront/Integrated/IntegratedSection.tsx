            ...
            <div className="grid grid-cols-2 gap-6 relative">
              {data.tools.map((tool, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-brand-light border border-zinc-100 shadow-sm">
                      <Icon className="h-6 w-6 text-brand-darker opacity-80" strokeWidth={1.5} />
                    </div>
                    <div className="flex flex-col gap-2">
                       <h3 className="font-display text-xl font-medium tracking-tight text-[#111111]">
                         {tool}
                       </h3>
                       {!isFirst && <span className="font-ui text-sm text-[#111111]/40">Integrated by Default</span>}
                    </div>
                  </motion.div>
                );
              })}
