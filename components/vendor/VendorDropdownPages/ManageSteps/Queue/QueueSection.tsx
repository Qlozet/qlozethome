...
import { ListOrdered, Clock, Calendar, Timer } from "lucide-react";
...
export function QueueSection({ data }: QueueSectionProps) {
    ...
    <section className="relative z-10 bg-white px-6 py-24 lg:py-40" data-theme="light">
      <div className="mx-auto max-w-[94rem]">
        ...
        <div className="mb-24 flex flex-col items-center gap-8 text-center lg:mb-32">
          ...
          >
            <Timer className="h-6 w-6 text-black/40" strokeWidth={1.5} />
          </motion.div>
          <motion.span
          ...
          {data.features.map((feature, index) => {
            ...
            return (
              <motion.div
                ...
                whileInView="visible"
                viewport={{ once: true }}
                className="group relative flex flex-col items-center gap-10 overflow-hidden rounded-[3rem] bg-[#FAFAFA] p-12 text-center transition-all duration-700 hover:bg-zinc-50 hover:shadow-2xl hover:shadow-black/5"
              >
                {/* Icon Container */}
                ...
                {/* Abstract Interactive Detail - Countdown visual */}
                <div className="relative mt-auto w-full h-24 overflow-hidden rounded-2xl bg-white/50 border border-black/5 p-6 flex flex-col items-center justify-center gap-3">
                   <div className="flex gap-2">
                     {[1, 2, 3].map((i) => (
                       <motion.div 
                         ...
                         className="h-2 w-8 bg-black/10 rounded-full"
                       />
                     ))}
                   </div>
