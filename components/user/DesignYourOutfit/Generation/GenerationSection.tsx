              ...
              {data.features.map((feature, i) => {
                ...
                return (
                  <motion.div key={feature.title} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 + i * 0.1 }} className="flex items-center gap-4 group">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10 border border-white/10 text-white group-hover:bg-brand-darker group-hover:text-white transition-all shadow-sm">
              ...
              <div className="flex items-center justify-between pb-5 border-b border-white/5 mb-6">
                <div className="flex items-center gap-3">
                  <motion.div animate={{ rotate: generating ? 360 : 0 }} transition={{ duration: 2, repeat: generating ? Infinity : 0, ease: "linear" }}
                    className="h-8 w-8 flex items-center justify-center rounded-xl bg-white text-brand-darker">
              ...
              <div className="grid grid-cols-3 gap-3">
                {VARIATIONS.map((v, i) => (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={`relative aspect-square rounded-xl overflow-hidden cursor-pointer border-2 transition-all ${i === activeVar ? 'border-brand-darker shadow-lg' : 'border-white/10 opacity-50 hover:opacity-80'}`}
                  >
