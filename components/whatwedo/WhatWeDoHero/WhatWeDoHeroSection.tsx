  ...
  return (
    <section id={data.id} className="relative mt-5 h-[700px] w-full overflow-hidden">
      <div className="grid h-full w-full grid-cols-1 lg:grid-cols-2">
        {/* Left Background */}
        <div className="h-full w-full bg-[#7A7E0C]" />
        {/* Right Image */}
      ...
      <div className="absolute inset-0 z-10 mx-auto grid h-full max-w-[1440px] grid-cols-1 lg:grid-cols-2">
        ...
        <div className="flex h-full flex-col justify-center gap-4 px-6 py-8 sm:px-12 lg:gap-6 lg:px-16">
          ...
          <Link
            href={data.cta.href}
            className="inline-flex w-fit items-center gap-2 rounded-[5px] border border-white bg-transparent px-6 py-3 text-xs font-semibold uppercase tracking-[0.25em] text-white transition hover:bg-white hover:text-[#7A7E0C]"
          >
            {data.cta.label}
            <svg
